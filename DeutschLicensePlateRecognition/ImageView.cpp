...
void CImageView::DrawRectangle(CDC * pDC)
{
	...
	for (int i = 0; i < m_carPlateData.nPlate; i++) {
		int left = m_nLeft + (m_carPlateData.pPlate[i].rtPlate.left) * m_fScale;
		int right = m_nLeft + (m_carPlateData.pPlate[i].rtPlate.right) * m_fScale;
		int top = m_nTop + (m_carPlateData.pPlate[i].rtPlate.top) * m_fScale;
		int bottom = m_nTop + (m_carPlateData.pPlate[i].rtPlate.bottom) * m_fScale;
